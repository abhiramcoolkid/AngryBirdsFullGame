...
function setup(){
    ...
    arraybirds.push(bird4);
    arraybirds.push(bird3);
    arraybirds.push(bird2);
...
function mouseDragged(){
    if (gameState!=="launched"){
        Matter.Body.setPosition(arraybirds[arraybirds.length-1].body, {x: mouseX , y: mouseY});
        Matter.Body.applyForce(arraybirds[arraybirds.length-1].body,arraybirds[arraybirds.length-1].body.position,{x:5,y:-5});
...
function keyPressed(){
    if(keyCode === 32&&gameState==='launched'){
        if(arraybirds.length>0){
            Matter.Body.setPosition(arraybirds[arraybirds.length-1].body,{x:200,y:50});    
            slingshot.attach(arraybirds[arraybirds.length-1].body);
